#!/usr/bin/env python3
"""Ministerienyt og Styrelsesnyt: samlede officielle nyhedsarkiver.

Programmet henter officielle nyheder fra de konfigurerede departementer,
styrelser og myndigheder, gemmer separate arkiver fra 1. januar 2026 og bygger:
...
REJECTION_REASON_DA = {
    ...
    "source_filter_mismatch": "Artiklen tilhører ikke den valgte myndighed",
}
...
class SourceStatus:
    ...
    expected_candidate_count: float | None = None
    candidate_volume_ratio: float | None = None
    expected_articles_30_days: float | None = None
...
def public_url_for_source(url: str, source: dict) -> str:
    ...
    rewrites = source.get("public_path_rewrites", {})
    if isinstance(rewrites, dict):
        for old_prefix, new_prefix in rewrites.items():
            old_prefix = str(old_prefix)
            if parsed_normalized.path.casefold().startswith(old_prefix.casefold()):
                rewritten_path = str(new_prefix) + parsed_normalized.path[len(old_prefix):]
...
def looks_like_article(url: str, source: dict) -> bool:
    ...
    if article_url_regex:
        try:
            ...
            if not re.search(article_url_regex, path_and_query, flags=re.IGNORECASE):
                return False
        except re.error:
            return False
...
def listing_link_candidate(anchor, target: str, current_url: str, source: dict) -> bool:
    if source.get("disable_listing_pagination"):
        return False
        ...
        for listing_soup, follow_pagination, method in listing_soups:
            ...
            for anchor in listing_soup.find_all("a", href=True):
                ...
                if looks_like_article(target, source):
                    ...
                    if source.get("require_listing_date") and not published:
                        continue
                    candidate = Candidate(
                        url=target,
                        title=title,
...
def collect_feed_items(
    ...
    used = False
    requires_article_filter = bool(
        source.get("required_article_text") or source.get("excluded_article_text")
    )
    for feed_url in feed_candidate_urls(source, feed_urls):
        ...
        for entry in feed.entries:
            # Delte nyhedsarkiver skal filtreres på selve artikelsiden. De samme
            # URL'er findes via HTML-listen og bliver behandlet dér.
            if requires_article_filter:
                continue
            published = None
...
def source_text_filter_matches(source: dict, text: str) -> bool:
    """Afgræns delte officielle arkiver til den myndighed, der udgav artiklen."""
    folded = clean_text(text).casefold()
    required = source.get("required_article_text", [])
    excluded = source.get("excluded_article_text", [])
    if isinstance(required, str):
        required = [required]
    if isinstance(excluded, str):
        excluded = [excluded]
    required_terms = [clean_text(str(value)).casefold() for value in required if clean_text(str(value))]
    excluded_terms = [clean_text(str(value)).casefold() for value in excluded if clean_text(str(value))]
    if required_terms and not any(term in folded for term in required_terms):
        return False
    if excluded_terms and any(term in folded for term in excluded_terms):
        return False
    return True
...
def item_from_candidate(
    ...
    must_fetch = (
        ...
        or bool(source.get("required_article_text"))
        or bool(source.get("excluded_article_text"))
        or not published
    ...
    if source.get("required_article_text") or source.get("excluded_article_text"):
        article_text = article_soup.get_text(" ", strip=True) if article_soup is not None else ""
        if not source_text_filter_matches(source, article_text):
            record_rejection(
                source["name"], title, final_url, "source_filter_mismatch",
...
def collect_verified_archive_items(
    source: dict,
    known_urls: set[str],
    status: SourceStatus,
) -> tuple[list[Item], bool]:
    """Indlæs et lokalt, manuelt verificeret grundarkiv som crawler-reserve.

    Funktionen bruges kun til kilder, hvor den officielle liste kan afvise
    GitHub Actions, selv om den virker i en almindelig browser. Grundarkivet
    indeholder udelukkende poster kontrolleret mod den officielle kildeliste;
    en normal live-crawl kan fortsat supplere det med nyere poster.
    """
    archive_name = clean_text(str(source.get("verified_archive_file", "")))
    if not archive_name:
        return [], False
    archive_path = (Path(__file__).resolve().parent / archive_name).resolve()
    project_root = Path(__file__).resolve().parent
    if archive_path.parent != project_root:
        append_error(status, "Det verificerede grundarkiv skal ligge i projektets rod.")
        return [], False
    try:
        payload = json.loads(archive_path.read_text(encoding="utf-8"))
        if not isinstance(payload, dict) or payload.get("source") != source["name"]:
            raise ValueError("kildenavnet matcher ikke konfigurationen")
        rows = payload.get("items", [])
        if not isinstance(rows, list) or not rows:
            raise ValueError("arkivet indeholder ingen poster")
    except Exception as exc:
        append_error(status, f"Verificeret grundarkiv kunne ikke indlæses: {exc}")
        return [], False
    ...
    for row in rows:
        ...
        url = normalize_url(str(row.get("url", "")), keep_query=True)
        published = parse_date(str(row.get("published", "")))
        if not title or not url or not published or published < ARCHIVE_START:
            continue
        if not looks_like_article(url, source):
            continue
        candidate_count += 1
        key = canonical_url(url)
        if key in known_urls:
            status.known_candidates_skipped += 1
            continue
        description = clean_text(str(row.get("description", "")))[:900]
        result[key] = with_item_identity(
            Item(source["name"], title, url, published, description),
            first_seen_at=datetime.now(timezone.utc),
        )
    ...
    if candidate_count != len(rows):
        append_error(
            status,
            f"Verificeret grundarkiv indeholdt {len(rows) - candidate_count} ugyldige poster.",
        )
        return [], False
    status.listing_pages += 1
    status.article_candidates += candidate_count
    status.methods.append("Verificeret ERST-arkiv")
...
def collect_source(
    ...
    *,
    ...
    verified_items, verified_ok = collect_verified_archive_items(source, known_urls, status)
    verified_candidate_count = status.article_candidates if verified_ok else 0

    ritzau_items, ritzau_ok = collect_ritzau_items(session, source, known_urls, status)
    ritzau_candidate_count = (
        status.article_candidates - verified_candidate_count if ritzau_ok else 0
    )
    ...
    elif verified_ok and source.get("verified_archive_ignores_cloudflare_403"):
        # ERSTs Cloudflare-regel afviser jævnligt GitHub Actions med en kendt
        # 403-challenge. Det verificerede grundarkiv er netop sikkerhedsnettet
        # for denne situation, så den forventede afvisning skal ikke vises som
        # en kildefejl. Andre fejl bevares og er fortsat synlige i diagnostikken.
        status.errors = [
            message for message in (status.errors or [])
            if not (
                message.startswith("Liste kunne ikke hentes:")
                and "erhvervsstyrelsen.dk/nyheder" in message
                and "403 Client Error" in message
            )
        ]
    ...
    for key, candidate in sitemap_candidates.items():
        candidates[key] = merge_candidate(candidates.get(key), candidate)
    status.article_candidates = (
        len(candidates) + verified_candidate_count + ritzau_candidate_count
    )
...
def evaluate_source_self_test(status: SourceStatus, previous_state: dict | None = None) -> None:
    ...
    reliable_volume_check = not (
        status.fast_mode
        or status.pagination_limited
        or status.historical_pages_skipped
        or status.sitemap_skipped_by_cache
    )
    expected_raw = previous_state.get(
        "expected_candidate_count",
        previous_state.get("last_full_candidate_count", 0),
    )
    try:
        ...
        expected = 0.0
    if expected > 0:
        status.expected_candidate_count = round(expected, 1)
        status.candidate_volume_ratio = round(status.article_candidates / expected, 3)
    if (
        reliable_volume_check
        and expected >= 10
        and status.article_candidates < expected * 0.55
    ):
        status.quality_flags.append(
            f"Kandidatniveauet er {status.article_candidates} mod forventet ca. {expected:.0f}."
        )
...
CONTENT_STOPWORDS = {
    "alle", "andre", "bliver", "blev", "bruge", "danske", "den", "der", "det",
    "dette", "efter", "eller", "fra", "faar", "for", "fordi", "har", "hos", "ikke",
    "kan", "med", "mere", "mod", "nye", "nyt", "ogsaa", "om", "over", "paa", "skal",
    "som", "til", "under", "ved", "vil", "vores", "være", "var", "samt",
}


def significant_content_tokens(value: str, source_name: str = "") -> set[str]:
    """Centrale ord til et konservativt internt sammenhængstjek."""
    def latinize(text: str) -> str:
        return text.casefold().replace("æ", "ae").replace("ø", "oe").replace("å", "aa")

    normalized = unicodedata.normalize("NFKD", latinize(clean_text(value)))
    ascii_value = "".join(char for char in normalized if not unicodedata.combining(char))
    source_tokens = {
        token for token in re.findall(r"[a-z0-9]+", unicodedata.normalize("NFKD", latinize(source_name)))
        if len(token) >= 4
    }
    return {
        token for token in re.findall(r"[a-z0-9]+", ascii_value)
        if len(token) >= 4 and token not in CONTENT_STOPWORDS and token not in source_tokens
    }


def article_content_consistency_flags(item: Item) -> list[str]:
    """Find tydelige rubrik-/manchet-/link-brud uden at afvise artiklen.

    Resultatet bruges kun i den interne diagnostik. Kontrollen er bevidst
    forsigtig, fordi en god manchet ikke altid gentager rubrikkens ord.
    """
    title_tokens = significant_content_tokens(item.title, item.source)
    slug = urlparse(item.url).path.rstrip("/").rsplit("/", 1)[-1].replace("-", " ")
    slug_tokens = significant_content_tokens(slug, item.source)
    flags: list[str] = []
    title_slug_overlap = title_tokens & slug_tokens
    if (
        len(title_tokens) >= 2
        and len(slug_tokens) >= 3
        and not title_slug_overlap
        and not re.fullmatch(r"\d+", slug.replace(" ", ""))
    ):
        flags.append("Rubrik og linksti deler ingen centrale ord.")
    return flags


def source_content_consistency_warnings(items: list[Item]) -> list[tuple[Item, list[str]]]:
    """Find især forskudte listekort, hvor en manchet tilhører en anden rubrik."""
    title_tokens = [significant_content_tokens(item.title, item.source) for item in items]
    slug_tokens = [
        significant_content_tokens(
            urlparse(item.url).path.rstrip("/").rsplit("/", 1)[-1].replace("-", " "),
            item.source,
        )
        for item in items
    ]
    result: list[tuple[Item, list[str]]] = []
    for index, item in enumerate(items):
        flags: list[str] = []
        description_tokens = significant_content_tokens(item.description, item.source)
        own_overlap = len(title_tokens[index] & description_tokens)
        best_other_overlap = max(
            (
                len(other_tokens & description_tokens)
                for other_index, other_tokens in enumerate(title_tokens)
                if other_index != index
            ),
            default=0,
        )
        if (
            len(description_tokens) >= 5
            and len(title_tokens[index] & slug_tokens[index]) >= 2
            and own_overlap == 0
            and best_other_overlap >= 4
        ):
            flags.append("Manchetten matcher en anden rubrik i samme kilde bedre.")
        own_link_overlap = len(title_tokens[index] & slug_tokens[index])
        best_other_link_overlap = max(
            (
                len(title_tokens[index] & other_slug_tokens)
                for other_index, other_slug_tokens in enumerate(slug_tokens)
                if other_index != index
            ),
            default=0,
        )
        if (
            len(title_tokens[index]) >= 3
            and best_other_link_overlap >= 3
            and best_other_link_overlap >= own_link_overlap + 2
        ):
            flags.append("Rubrikken matcher en anden linksti i samme kilde bedre.")
        if flags:
...
def annotate_silence_warnings(statuses: list[SourceStatus], items: list[Item]) -> None:
    ...
    for status in statuses:
        ...
        recent_items = [
            ...
            if 0 <= (today - item.published.date()).days <= 90
        ]
        status.expected_articles_30_days = round(len(recent_items) / 3, 1)
        publication_days = sorted({item.published.date() for item in recent_items})
        ...
        if status.silence_warning:
            status.quality_flags.append(
                f"Ingen artikel i {status.days_since_last_publication} dage; normal tærskel er {threshold} dage."
            )
...
def update_source_state(previous_payload: dict, statuses: list[SourceStatus]) -> dict:
    ...
    for status in statuses:
        ...
        if (
            ...
            previous_expected_raw = old.get("expected_candidate_count", 0)
            try:
                previous_expected = float(previous_expected_raw or 0)
            except (TypeError, ValueError):
                previous_expected = 0.0
            current_candidates = max(0, int(status.article_candidates or 0))
            if current_candidates > 0:
                row["expected_candidate_count"] = round(
                    current_candidates
                    if previous_expected <= 0
                    else previous_expected * 0.8 + current_candidates * 0.2,
                    1,
                )
        if ok:
...
def build_diagnostics_html(diagnostics: dict, alerts: dict, audit: dict) -> str:
    ...
    for row in diagnostics.get("sources", []):
        notes = list(row.get("self_test_notes", []) or [])
        notes.extend(row.get("quality_flags", []) or [])
        notes = " · ".join(dict.fromkeys(notes))
        expected = row.get("expected_articles_30_days")
        expected_text = "–" if expected is None else str(expected)
        rejection_rate = round(float(row.get("rejection_rate", 0) or 0) * 100)
        review_class = ' class="review"' if row.get("review_recommended") else ""
        source_rows.append(
            f"<tr{review_class}>"
            f"<td>{e(row.get('source'))}</td>"
            f"<td><strong>{e(row.get('quality_score'))}</strong></td>"
            f"<td>{e(row.get('self_test'))}</td>"
            f"<td>{e(row.get('article_candidates'))}</td>"
            f"<td>{e(expected_text)}</td>"
            f"<td>{e(row.get('known_candidates_skipped'))}</td>"
            f"<td>{e(row.get('fresh_items'))}</td>"
            f"<td>{e(row.get('rejected_during_current_crawl'))} ({e(rejection_rate)} %)</td>"
            f"<td>{e(row.get('content_consistency_warnings'))}</td>"
    ...
    return f'''<!doctype html><html lang="da"><head><meta charset="utf-8"><meta name="robots" content="noindex,nofollow,noarchive"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Ministerienyt diagnostik</title><style>body{{font:14px/1.45 system-ui,sans-serif;margin:28px;color:#18222c}}h1,h2{{margin:.4em 0}}.meta{{color:#5d6974}}.table-wrap{{overflow:auto}}table{{border-collapse:collapse;width:100%;margin-top:12px}}th,td{{border:1px solid #dce2e7;padding:7px 8px;text-align:left;vertical-align:top}}th{{background:#f1f3f5}}tr.review{{background:#fff7e6}}code{{background:#f1f3f5;padding:1px 4px}}</style></head><body><h1>Ministerienyt – intern kvalitetsrapport</h1><p class="meta">Genereret {e(diagnostics.get('generated_at'))}. Denne fil ligger kun i repositoryet og publiceres ikke som en del af GitHub Pages.</p><p>Arkiv: <strong>{e(diagnostics.get('archive_items'))}</strong> · viste historier: <strong>{e(diagnostics.get('display_items_after_deduplication'))}</strong> · kilder OK: <strong>{e(diagnostics.get('healthy_sources'))}/{e(diagnostics.get('source_count'))}</strong> · anbefalet kontrol: <strong>{e(diagnostics.get('sources_recommended_for_review'))}</strong> · kørsel: <strong>{e(diagnostics.get('runtime_seconds'))} sek.</strong></p><h2>Driftsalarmer</h2>{alert_html}<h2>Kildekvalitet</h2><p class="meta">Kvalitetsscoren er en intern prioriteringshjælp. Den påvirker ikke artiklerne eller den offentlige kildestatus.</p><div class="table-wrap"><table><thead><tr><th>Kilde</th><th>Score</th><th>Selvtest</th><th>Kandidater</th><th>Forv. artikler/30 dage</th><th>Kendte sprunget over</th><th>Nye/opdaterede</th><th>Afvist</th><th>Sammenhængstjek</th><th>Sek.</th><th>Noter</th></tr></thead><tbody>{''.join(source_rows)}</tbody></table></div><h2>Kildeaudit</h2><p>Seneste audit: <code>{e(audit.get('generated_at'))}</code> · fuld audit: <strong>{'ja' if audit.get('full_audit') else 'nej'}</strong>.</p></body></html>'''


def build_rss(
    entries: Iterable[DisplayEntry],
    site_url: str,
    feed_url: str,
    ...
    rss_title = clean_text(str(ui_config.get("rss_title", f"{site_name} – officielle nyheder")))
    rss_description = clean_text(str(ui_config.get(
        "rss_description",
    ...
    ET.SubElement(channel, "generator").text = f"{site_name} {APP_VERSION}"
    if feed_url:
...
def build_html(
    ...
    *,
    ...
    page_heading = clean_text(str(ui_config.get(
        "page_heading",
        "Nyheder fra danske styrelser og myndigheder" if agency_mode else "Nyheder fra danske ministerier",
    )))
    page_description = clean_text(str(ui_config.get(
        "page_description",
        "Samlet arkiv over officielle nyheder fra danske styrelser og myndigheder siden 1. januar 2026."
        if agency_mode else
        "Samlet arkiv over officielle nyheder fra danske ministerier og Regeringen.dk siden 1. januar 2026.",
    )))
    favorites_label = clean_text(str(ui_config.get(
        "favorites_label", "Mine myndigheder" if agency_mode else "Mine ministerier"
    )))
    favorites_help = clean_text(str(ui_config.get(
        "favorites_help",
        "Vælg myndigheder nedenfor. Brug derefter knappen til kun at vise deres nyheder."
        if agency_mode else
        "Vælg ministerier nedenfor. Brug derefter knappen til kun at vise deres nyheder.",
    )))
    storage_namespace = re.sub(
        r"[^a-z0-9-]+", "-", str(ui_config.get("storage_namespace", site_name)).casefold()
    ).strip("-") or "ministerienyt"
    ministry_href = clean_text(str(ui_config.get("ministerienyt_href", "./"))) or "./"
    agency_href = clean_text(str(ui_config.get("styrelsesnyt_href", "styrelsesnyt/"))) or "styrelsesnyt/"
    dedup_explanation = clean_text(str(ui_config.get(
        "dedup_explanation",
        "Samme historie fra flere styrelser eller myndigheder samles i ét kort."
        if agency_mode else
    ...
    for name in ministries:
        ...
        parent_cell = (
            f'<td>{esc(clean_text(str(source.get("responsible_ministry", ""))))}</td>'
            if agency_mode else ""
        )
        observed_types = content_types_by_source.get(name, set())
        if not observed_types:
            observed_types = {clean_text(str(source.get("default_article_type", "Nyhed"))) or "Nyhed"}
        content_profile = ", ".join(sorted(observed_types, key=str.casefold))
        source_rows.append(
            f'''<tr><td><a href="{esc(source.get('home_url', source['start_urls'][0]))}" target="_blank" rel="noopener noreferrer">{esc(name)}</a></td>{parent_cell}<td>{esc(content_profile)}</td><td>{raw_counts.get(name, 0)}</td><td>{state}</td><td>{esc(methods)}</td><td>{note_html}</td></tr>'''
        )
    ...
    social_meta = (
        ...
        f'<meta property="og:title" content="{esc(site_name)}"><meta property="og:description" content="{esc(page_description)}">'
        f'<meta property="og:url" content="{canonical_href}"><meta property="og:image" content="{social_image}">'
        f'<meta name="twitter:card" content="summary"><meta name="twitter:title" content="{esc(site_name)}">'
...
(() => {
  ...
  const topicsOnly = document.getElementById('topics-only');
  ...
  function syncFavoritesUI() {
    ...
    if (favoritesSummary) favoritesSummary.textContent = favorites.size ? '★ ' + FAVORITES_LABEL + ' (' + favorites.size + ')' : '★ ' + FAVORITES_LABEL;
    mineOnly.textContent = mineOnly.getAttribute('aria-pressed') === 'true' ? 'Viser kun mine' : 'Vis kun mine';
  }

  function parseTopics(value) {
    return [...new Set((value || '').split(/[,;\n]+/).map(norm).filter(Boolean))].slice(0, 20);
  }

  function saveTopics() {
    try { localStorage.setItem(TOPICS_KEY, JSON.stringify(topics)); } catch (error) {}
  }

  function syncTopicsUI() {
    const countText = topics.length === 1 ? '1 emne' : topics.length + ' emner';
    topicsSummary.textContent = topics.length ? 'Mine emner (' + topics.length + ')' : 'Mine emner';
    topicsPreview.textContent = topics.length ? countText + ': ' + topics.join(', ') : 'Ingen emner gemt';
    topicsInput.value = topics.join(', ');
    topicsOnly.textContent = topicsOnly.getAttribute('aria-pressed') === 'true' ? 'Viser mine emner' : 'Vis mine emner';
  }
  ...
  async function applyFilters(resetLimit = false) {
    ...
    const onlyTopics = topicsOnly.getAttribute('aria-pressed') === 'true';
    ...
    onlyTopics ? url.searchParams.set('emner', '1') : url.searchParams.delete('emner');
    url.searchParams.delete('emne');
    if (onlyTopics) {
  ...
  function toggleTopics() {
    ...
    topicsOnly.setAttribute(
      'aria-pressed',
      topicsOnly.getAttribute('aria-pressed') === 'true' ? 'false' : 'true'
    );
    syncTopicsUI();
    applyFilters(true);
  }
  ...
  topicsOnly.addEventListener('click', toggleTopics);
  topicsForm.addEventListener('submit', event => {
    event.preventDefault();
    topics = parseTopics(topicsInput.value);
    if (!topics.length) topicsOnly.setAttribute('aria-pressed', 'false');
    saveTopics();
    syncTopicsUI();
    applyFilters(true);
  });
  clearTopics.addEventListener('click', () => {
    topics = [];
    topicsOnly.setAttribute('aria-pressed', 'false');
    saveTopics();
    syncTopicsUI();
    applyFilters(true);
  });
  ...
  document.addEventListener('keydown', event => {
    ...
    if (event.key === 'Escape') {
      ...
      if (topicsMenu.open) topicsMenu.open = false;
      if (search.value) {
        search.value = '';
        applyFilters(true);
      }
...
'''
    ...
    changelog_html = changelog_html.replace(
        ...
        '<li>Ministerienyt og Styrelsesnyt kan vælges direkte ved siden af hinanden i topbjælken.</li>'
        '<li>Dubletter samles inden for hver side, mens samme historie fortsat kan fremgå én gang på begge hovedsider.</li></ul>'
    ...
    ministry_active = " active" if not agency_mode else ""
    agency_active = " active" if agency_mode else ""
    ministry_current = ' aria-current="page"' if not agency_mode else ""
    agency_current = ' aria-current="page"' if agency_mode else ""
    brand_navigation = (
        '<nav class="brand-nav" aria-label="Hovedsider">'
        f'<a class="brand-link{ministry_active}" href="{esc(ministry_href)}"{ministry_current}>Ministerienyt</a>'
        f'<a class="brand-link{agency_active}" href="{esc(agency_href)}"{agency_current}>Styrelsesnyt</a>'
        '</nav>'
    )
...
def diagnostics_payload(
    ...
    source_lookup = {source["name"]: source for source in (sources or [])}
    ...
    for status in statuses:
        ...
        crawl_decisions = rejected_count + max(0, int(status.fresh_items or 0))
        rejection_rate = round(rejected_count / crawl_decisions, 3) if crawl_decisions else 0.0
        consistency_rows = source_content_consistency_warnings(source_items)
        consistency_count = len(consistency_rows)
        consistency_samples = [
            {"title": item.title, "url": item.url, "flags": flags}
            for item, flags in consistency_rows[:5]
        ]
        quality_flags = list(status.quality_flags or [])
        if rejection_rate >= 0.75 and rejected_count >= 5:
            quality_flags.append(
                f"{rejected_count} kandidater blev frasorteret ({round(rejection_rate * 100)} % af crawlens afgørelser)."
            )
        configured_mismatch_threshold = source_lookup.get(status.name, {}).get(
            "content_consistency_review_threshold"
        )
        try:
            mismatch_threshold = max(1, int(configured_mismatch_threshold))
        except (TypeError, ValueError):
            mismatch_threshold = max(2, int(len(source_items) * 0.1))
        if consistency_count >= mismatch_threshold:
            quality_flags.append(
                f"{consistency_count} arkivposter bør stikprøvekontrolleres for rubrik, manchet eller link."
            )
        quality_flags = list(dict.fromkeys(quality_flags))
        candidate_volume_warning = any(
            flag.startswith("Kandidatniveauet er ") for flag in quality_flags
        )
        quality_score = 100
        if status.self_test == "fail":
            quality_score -= 40
        elif status.self_test == "warn":
            quality_score -= 20
        if candidate_volume_warning:
            quality_score -= 20
        if rejection_rate >= 0.75 and rejected_count >= 5:
            quality_score -= 15
        if consistency_count >= mismatch_threshold:
        ...
        rows.append({
            ...
            "expected_articles_30_days": status.expected_articles_30_days,
            "expected_candidate_count": status.expected_candidate_count,
            "candidate_volume_ratio": status.candidate_volume_ratio,
            "candidate_volume_warning": candidate_volume_warning,
            ...
            "rejected_during_current_crawl": rejected_count,
            "rejection_rate": rejection_rate,
            "rejection_reasons": dict(sorted(reasons.items())),
            "content_consistency_warnings": consistency_count,
            "content_consistency_samples": consistency_samples,
            "quality_score": quality_score,
            "quality_flags": quality_flags,
            "review_recommended": bool(
                status.self_test in {"warn", "fail"} or quality_flags
            ),
    ...
    review_count = sum(1 for row in rows if row.get("review_recommended"))
    return {
        "version": APP_VERSION,
        "generated_at": datetime.now(timezone.utc).isoformat(),
        ...
        "sources_recommended_for_review": review_count,
        "sources": rows,
    }
...
def generate_pwa_assets(site_dir: Path, ui_config: dict | None = None) -> None:
    ui_config = ui_config or {}
    site_name = clean_text(str(ui_config.get("site_name", "Ministerienyt"))) or "Ministerienyt"
    page_description = clean_text(str(ui_config.get(
        "page_description",
        "Samlet overblik over officielle nyheder fra danske ministerier og Regeringen.dk.",
    )))
    storage_namespace = re.sub(
        r"[^a-z0-9-]+", "-", str(ui_config.get("storage_namespace", site_name)).casefold()
    ).strip("-") or "ministerienyt"
    ...
    service_worker = r'''const CACHE = '{namespace}-v{version}';
...
def load_site_config(path: Path) -> dict:
    defaults = {
        "site_name": "Ministerienyt",
        "site_kind": "ministries",
        "page_heading": "Nyheder fra danske ministerier",
        "page_description": "Samlet arkiv over officielle nyheder fra danske ministerier og Regeringen.dk siden 1. januar 2026.",
        "rss_title": "Ministerienyt – nyheder fra danske ministerier og Regeringen.dk",
        "rss_description": "Samlet arkiv med officielle nyheder og pressemeddelelser fra 1. januar 2026.",
        "favorites_label": "Mine ministerier",
        "favorites_help": "Vælg ministerier nedenfor. Brug derefter knappen til kun at vise deres nyheder.",
        "storage_namespace": "ministerienyt",
        "ministerienyt_href": "./",
        "styrelsesnyt_href": "styrelsesnyt/",
        "noindex": False,
        "goatcounter_code": "",
...
def load_sources_config(path: Path) -> list[dict]:
    """Læs både den oprindelige kildeliste og en liste med fælles standarder."""
    raw = json.loads(path.read_text(encoding="utf-8"))
    ...
    if any(not name for name in names):
        raise ValueError(f"{path} indeholder en kilde uden navn.")
...
def main() -> int:
    ...
    print(f"Dubletkontrol: {duplicates_merged} kildedubletter samlet.", file=sys.stderr)
    ...
    generate_pwa_assets(html_output.parent, config)

    elapsed = time.monotonic() - started
    diagnostics = diagnostics_payload(
        statuses, merged, display_entries, duplicates_merged, elapsed, sources
    )
    ...
    if args.full_audit or not source_audit_output.exists():
        ...
        try:
            ...
            audit = current_audit
    site_name = clean_text(str(config.get("site_name", "Ministerienyt"))) or "Ministerienyt"
    diagnostics_html = build_diagnostics_html(diagnostics, alerts, audit)
    if site_name != "Ministerienyt":
        diagnostics_html = diagnostics_html.replace("Ministerienyt", site_name)
    diagnostics_html_output.write_text(diagnostics_html, encoding="utf-8")
